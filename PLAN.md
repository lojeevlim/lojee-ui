...
### Phase 3 — Styling strategy (the trickiest decision) — **decided: Option 1**
...
2. ~~Ship a prebuilt CSS file (`dist/style.css`) generated by running Tailwind's build against the library's own source, and have consumers `import "lojee-ui/style.css"` once~~ — tried first, then dropped: baking a full compiled stylesheet defeated the point of keeping Tailwind out of the published bundle, and it wasn't tree-shaken.
...
- **Decision: Option 1.** Consumers own their own Tailwind v4 install/config and `@source`-scan `node_modules/lojee-ui/dist`; `tailwindcss` and `lucide-react` are `peerDependencies` (alongside `react`/`react-dom`), not bundled. The package still ships one small `dist/theme.css` — not a compiled stylesheet, just the two `@theme`/`@keyframes` tokens Tailwind's static scanner can't infer on its own (`Loader`'s shimmer animation, `ProgressBar`'s indeterminate mode) — which consumers `@import` once alongside their own `@import "tailwindcss"`. See the README "Installation" section for the exact snippet. Revisit option 3 only if consumers hit real friction with this.
...
- Add `"peerDependencies"`: `react`, `react-dom`, `lucide-react`, `tailwindcss` (with sensible semver ranges) so consumers control their own installed versions instead of getting duplicates, and so Tailwind/lucide-react are never bundled into the published package (see Phase 3 and the "Dependency policy" note below). The `lojee-ui/elements` entry bundles its own React + ReactDOM + `@r2wc/react-to-web-component` + Tailwind CSS internally (see Phase 2) rather than relying on the consumer's setup — it's meant for non-React apps.

#### Dependency policy (for future UI-adjacent packages)
The same "consumer installs it, we don't bundle it" pattern applies to any future dependency that's UI-framework-ish (icons, animation, headless primitives — e.g. `framer-motion`, a Radix primitive), but **only for the React lib build** (`vite.lib.config.ts` → `dist/index.js`/`dist/index.cjs`):
1. Add the package to `peerDependencies` (and `devDependencies`, for building this repo locally) — not `dependencies`.
2. Add it to `vite.lib.config.ts`'s `rollupOptions.external` array so Rollup doesn't inline it into the bundle.
3. Document the new peer requirement in the README "Installation" section.
- This does **not** apply to the `lojee-ui/elements` (Web Components) build — that target intentionally stays self-contained/bundled (including React itself and Tailwind's compiled CSS), since it's meant for non-React consumers to drop in via a single `<script type="module">` with zero setup. See the Web Components "Bundle-size tradeoff" note above.
...
2. ~~**Styling strategy**~~ — **resolved:** Option 1 (Phase 3). Consumers own their Tailwind v4 config and `@source`-scan `node_modules/lojee-ui/dist`; `tailwindcss` and `lucide-react` are `peerDependencies`, not bundled. `lojee-ui` ships only `dist/theme.css` for the two extra animation tokens.
